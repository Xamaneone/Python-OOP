...
class System:
    ...
    @staticmethod
    def analyze():
        return f"System Analysis\nHardware Components: {len(System._hardware)}\nSoftware Components: {len(System._software)}\nTotal Operational Memory: {sum([h.used_capacity for h in System._hardware])} / {sum([h.memory for h in System._hardware])}\nTotal Capacity Taken: {sum(h.used_capacity for h in System._hardware)} / {'Fix me'}"
    ...
    def system_split():
        ...
        for h in System._hardware:
            ...
            express_software_components = [s for s in h.software if s.__class__.__name__ == "ExpressSoftware"]
            ...
            result += f"Capacity Usage: {sum([s.capacity_consumption for s in h.software_components])} \ {h.software}\n"
    ...
    def available_capacity(self):
        return self.available_capacity - self.used_capacity

    @property
    def used_capacity(self):
        return sum([s.capacity_consumption for s in self._hardware])
